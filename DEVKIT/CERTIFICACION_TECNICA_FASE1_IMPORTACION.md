...
Una tercera comprobación limpia localizó una omisión de metadatos en `fase1-download-clean-e2e`: el mapa `recipe_names` no se escribía en `e2e-state.json`, aunque los 52 nombres sí estaban en las recetas y en el XLSX descargado. El preparador publica ahora ese mapa para que la aserción compare dos fuentes reales. La descarga limpia pasa 1/1 y la regresión E2E relacionada confirma contrato público 1/1, subida XLSX 1/1, rehidratación 1/1 y persistencia con artículo/referencia 2/2.

## Hotfix final de aprovisionamiento Python reproducible (6 de septiembre de 2026)

La auditoría del baseline publicado `c5279f4a74462c51c981a690fc5e301bb75474f4` confirmó que el repositorio ya utilizaba `API/requirements-http.txt`, por lo que se conserva ese sistema y no se introduce Poetry, uv ni otro gestor paralelo. El manifiesto runtime incorpora las dependencias directas de Excel y Starlette que usaba el código pero no declaraba. `API/requirements-dev.txt` extiende el runtime con `httpx` y `pytest`, las únicas dependencias adicionales observadas en los tests backend. `DEVKIT/BOOTSTRAP_ENTORNO_NUEVO.md` fija Python 3.13.x, documenta la versión certificada y elimina la ruta personal al venv histórico del README de API.

Evidencia previa a publicación: un venv nuevo con Python 3.13.14, sin `PYTHONPATH`, instaló exclusivamente `API/requirements-dev.txt` usando `--no-cache-dir`; `pip check`, imports, compilación y arranque HTTP aislado resultaron correctos. La regresión ampliada terminó con 281/281 pruebas y los E2E autocontenidos de contrato XLSX, lote físico de 52 recetas y sentinel económico pasaron 1/1 cada uno usando el Python de ese venv. No se modificó lógica de producto ni se ejecutó confirmación o WRITE sobre `DATOS` real. Fase 1.5 no se inicia.
