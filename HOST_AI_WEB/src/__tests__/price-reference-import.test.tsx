...
it("consolida, copia y prepara una importación determinista sin Web Search", async () => {
  ...
  vi.spyOn(articulosService, "importedReferencesState").mockResolvedValue({ ok: true, request_id: "R", api_version: "1", version: "1", modo_seguro: true, datos_reales_modificados: false, workflow: null });
  ...
  fireEvent.click(screen.getByRole("button", { name: "Copiar lista para enriquecimiento externo" })); await waitFor(() => expect(writeText).toHaveBeenCalledWith("ART-1 | Champiñones"));
  fireEvent.change(screen.getByLabelText("Reimportar precio y proveedor/tienda de referencia"), { target: { value: "article_id,precio\nART-1,2.50" } }); fireEvent.click(screen.getByRole("button", { name: "Preparar vista previa" }));
