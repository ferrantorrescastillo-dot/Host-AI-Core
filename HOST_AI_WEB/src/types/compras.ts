...
export type CompraDraft = { id: string; proveedor: string; estado: string; lineas: CompraDraftLine[]; importe_estimado: number; fecha?: string; referencia?: string; observaciones?: string; creado_en: string; actualizado_en: string; recepciones?: Array<{ id: string; estado: string; lineas?: number }>; origen: { tipo: string; id: string; version: number; propuesta_id: string } };
...
export type CompraDraftInput = { proveedor: string; proveedor_id?: string; fecha?: string; referencia?: string; observaciones?: string; lineas: CompraDraftLine[] };
...
export type ReceptionDocument = { document_id: string; nombre: string; tipo_mime: string; tamano: number; fecha_subida: string; usuario: string; proveedor: string; order_id: string; reception_id: string; referencia: string; fecha_albaran: string; observaciones: string; checksum_sha256: string; estado: "ADJUNTO" | "PENDIENTE_REVISION" | "REVISADO" | "DESCARTADO" };
export type PurchaseReception = { id: string; reception_id: string; order_id: string; proveedor: string; fecha: string; referencia: string; estado: "BORRADOR" | "CONFIRMADA"; actualizado_en: string; lineas: ReceptionLine[]; incidencias: Array<{ code: string; message: string; bloqueante: boolean; order_line_id?: string }>; observaciones: string; confirmable: boolean; document_id?: string; document_name?: string; document_type?: string; document_reference?: string; documento?: ReceptionDocument | null };
...
export type ReceptionDocumentResponse = ApiEnvelopeBase & { recepcion?: PurchaseReception; documento: ReceptionDocument & { contenido_base64?: string }; idempotente?: boolean; stock_modificado: false };
