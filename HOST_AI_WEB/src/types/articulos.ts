...
export type ReferenciaImportPreviewRow = { fila: number; article_id: string; articulo: string; referencia: PrecioReferencia; preview_token: string; incluir: boolean };
export type ReferenciasImportPreview = ApiEnvelope & { formato_detectado: "JSON" | "CSV" | "TSV" | "MARKDOWN" | "VACIO"; listas: ReferenciaImportPreviewRow[]; ambiguas: Record<string, unknown>[]; invalidas: Record<string, unknown>[]; resumen: { listas: number; ambiguas: number; invalidas: number }; requiere_confirmacion: true; coste_ia_usd: string };
