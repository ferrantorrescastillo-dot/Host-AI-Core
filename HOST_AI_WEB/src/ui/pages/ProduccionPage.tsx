...
function PlanCard({ plan }: { plan: ProductionPlan }) {
  ...
  return <article className="produccion-card"><header><div><h3>{plan.nombre || "Plan sin nombre"}</h3><p className="meta-line">Menú {plan.menu_id || "sin identificar"} · versión {plan.menu_version || "-"}</p></div><span className="evento-status">{plan.estado}</span></header>
    ...
    <div className="menu-actions">{proposalId ? <Link to="/compras">Ver propuesta existente</Link> : <button type="button" disabled={proposalBusy || plan.clasificacion.faltantes_conocidos === 0} onClick={() => void createProposal()}>{proposalBusy ? "Generando propuesta..." : "Generar propuesta de compra"}</button>}<Link to={`/produccion/${encodeURIComponent(plan.id)}/stock`}>Revisar stock</Link><Link to="/articulos">Resolver artículos</Link></div>
