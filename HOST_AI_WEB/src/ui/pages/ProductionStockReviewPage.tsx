...
function ResolutionCard({ item, planId, refresh }: { item: ProductionIngredient; planId: string; refresh: () => Promise<void> }) {
  ...
  const saveStock = async () => { if (item.unidad_base_sugerida && unit !== item.unidad_base) await saveUnit(); const response = await stockService.createMovement({ article_id: item.articulo_id, tipo: type, cantidad: Number(quantity), unidad: unit, ubicacion: location, lote: lot, caducidad: expiry, observaciones: notes, production_plan_id: planId, return_to: `/produccion/${planId}/stock` }); setMessage(response.mensaje); await refresh(); };
