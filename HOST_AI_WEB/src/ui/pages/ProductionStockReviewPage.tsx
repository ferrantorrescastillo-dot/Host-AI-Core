...
function ResolutionCard({ item, planId, refresh }: { item: ProductionIngredient; planId: string; refresh: () => Promise<void> }) {
  ...
  useEffect(() => { setUnit(item.unidad_base || ""); }, [item.articulo_id, item.unidad_base, item.unidad_base_sugerida]);
  const persistUnit = async () => { const detail = (await articulosService.get(item.articulo_id)).articulo; await articulosService.update(item.articulo_id, { nombre: detail.nombre, unidad_base: unit }); };
  const saveUnit = async () => { await persistUnit(); await refresh(); };
  const saveStock = async () => { const response = await stockService.createProductionMovement(planId, { article_id: item.articulo_id, tipo: type, cantidad: Number(quantity), unidad: unit, ubicacion: location, lote: lot, caducidad: expiry, observaciones: notes, production_plan_id: planId, return_to: `/produccion/${planId}/stock` }); setMessage(response.mensaje); await refresh(); };
  return <article className="produccion-card"><h3>{item.nombre}</h3><p>Artículo: {item.articulo_id || "Sin relacionar"}</p><p>Necesidad del escandallo: {amount(item.cantidad, item.unidad)}</p><p>Unidad del artículo: {item.unidad_base || "Pendiente"}</p>{item.unidad_base_sugerida ? <div className="draft-warning"><p>Unidad sugerida / pendiente de revisión. Puedes cambiarla antes de registrar inventario.</p><label>Cambiar unidad sugerida<select value={unit} onChange={(e) => setUnit(e.target.value)}>{["kg","g","l","ml","u"].map((u) => <option key={u}>{u}</option>)}</select></label></div> : null}{item.estado_resolucion === "CONVERSION_PENDIENTE" ? <p className="draft-warning">Conversión pendiente: la unidad confirmada del artículo no coincide con la necesidad del escandallo.</p> : null}{item.inventario_incompatible?.length ? <p className="draft-warning">Existe inventario histórico en otra unidad y no se ha reinterpretado: {item.inventario_incompatible.map((lot) => amount(lot.cantidad ?? null, lot.unidad || "")).join(", ")}. Requiere una corrección trazable en Stock.</p> : null}<p>Stock: {item.disponible === null ? "Desconocido" : amount(item.disponible, item.unidad)}</p><p>Faltante: {item.faltante === null ? "No calculado" : amount(item.faltante, item.unidad)}</p><strong>{item.estado_resolucion}</strong>
