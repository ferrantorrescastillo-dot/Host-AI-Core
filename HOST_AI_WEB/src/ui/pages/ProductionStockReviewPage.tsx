...
function ResolutionCard({ item, planId, refresh }: { item: ProductionIngredient; planId: string; refresh: () => Promise<void> }) {
  ...
  const saveStock = async () => { if (item.unidad_base_sugerida) await persistUnit(); const response = await stockService.createMovement({ article_id: item.articulo_id, tipo: type, cantidad: Number(quantity), unidad: unit, ubicacion: location, lote: lot, caducidad: expiry, observaciones: notes, production_plan_id: planId, return_to: `/produccion/${planId}/stock` }); setMessage(response.mensaje); await refresh(); };
  return <article className="produccion-card"><h3>{item.nombre}</h3><p>Artículo: {item.articulo_id || "Sin relacionar"}</p><p>Necesario: {amount(item.cantidad, item.unidad)}</p><p>Unidad base: {item.unidad_base || "Pendiente"}</p>{item.unidad_base_sugerida ? <div className="draft-warning"><p>Unidad sugerida / pendiente de revisión. Puedes cambiarla antes de registrar inventario.</p><label>Cambiar unidad sugerida<select value={unit} onChange={(e) => setUnit(e.target.value)}>{["kg","g","l","ml","u"].map((u) => <option key={u}>{u}</option>)}</select></label></div> : null}<p>Stock: {item.disponible === null ? "Desconocido" : amount(item.disponible, item.unidad)}</p><p>Faltante: {item.faltante === null ? "No calculado" : amount(item.faltante, item.unidad)}</p><strong>{item.estado_resolucion}</strong>
