...
function RecepcionesSection({ items, onSaved }: { items: ComprasResult["pedidos"]; onSaved: () => void }) {
  ...
  const confirm = async () => { if (!reception || confirming.current || reception.estado === "CONFIRMADA") return; if (!window.confirm("Se registrará en Stock exactamente lo recibido.")) return; confirming.current = true; setBusy(true); try { const response = await comprasService.confirmReception(reception.id); setReception(response.recepcion); setMessage(response.idempotente ? "La recepción ya estaba confirmada; Stock no se duplicó." : "Recepción confirmada y Stock actualizado."); onSaved(); } catch (error) { setMessage(error instanceof Error ? error.message : "No se pudo confirmar la recepción."); } finally { confirming.current = false; setBusy(false); } };
  const receivable = items.filter((item) => ["preparado", "enviado"].includes(item.estado));
