...
class PipelineHistoricoInteligenteStock(BasePipeline):
    nombre = "historico_inteligente_stock"
    descripcion = "Histórico y análisis de movimientos de stock."
    ...
    def _ejecutar(self, solicitud: SolicitudPipeline) -> ResultadoPipeline:
        ...
        if solicitud.accion == "registrar":
            d = self.core.historico_inteligente_stock.registrar_movimiento(p["movimiento"])
        ...
        if solicitud.accion == "registrar_desde_aplicacion":
            d = self.core.historico_inteligente_stock.registrar_desde_aplicacion(p["resultado_aplicacion"])
            return ResultadoPipeline(d.get("registrado", False), self.nombre, solicitud.accion, d["lectura_host_ai"], d, [], not d.get("registrado", False))
        if solicitud.accion == "analizar_articulo":
            d = self.core.historico_inteligente_stock.analizar_articulo(p["articulo_id"])
