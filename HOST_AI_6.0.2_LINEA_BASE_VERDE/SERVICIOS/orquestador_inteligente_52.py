...
class OrquestadorInteligente52:
    ...
    def __init__(self, base_dir: Optional[Path] = None):
        ...
        # Alias mantenido por compatibilidad: ambos apuntan al mismo contexto de sesión.
    ...
    def procesar(self, texto: str) -> Dict[str, Any]:
        ...
        # 5.4.10-5.4.13: una respuesta corta debe continuar el flujo activo
        # antes de pasar por conectores o clasificadores generales.
        gestion_contexto = self._procesar_contexto_permanente_5410_5413(texto)
        if gestion_contexto is not None:
            return gestion_contexto
    ...
    def _sincronizar_contexto_5410(self) -> Dict[str, Any]:
        """Garantiza compatibilidad con código histórico que reasigna contexto_activo_549."""
        if self.contexto_activo_549 is not self.gestor_contexto_5410.contexto:
            if self.contexto_activo_549:
                self.gestor_contexto_5410.contexto = self.contexto_activo_549
            else:
                self.contexto_activo_549 = self.gestor_contexto_5410.contexto
        return self.gestor_contexto_5410.contexto

    def _respuesta_contextual_5410(self, estado: str, mensaje: str, **datos: Any) -> Dict[str, Any]:
        return {
            "ok": True,
            "version": self.VERSION,
            "intencion": datos.pop("intencion", "continuidad_flujo"),
            "estado": estado,
            "mensaje": mensaje,
            "datos": datos,
            "pasos": self._sincronizar_contexto_5410().get("flujo", {}).get("pasos", []),
        }

    def _procesar_contexto_permanente_5410_5413(self, texto: str) -> Optional[Dict[str, Any]]:
        contexto = self._sincronizar_contexto_5410()
        if not self.gestor_contexto_5410.esta_activo():
            return None
        ...
        natural = resolver_referencia_natural_5413(texto, contexto)
        if natural.get("gestionado"):
            tipo_natural = natural.get("tipo")
            if tipo_natural == "consultar_pendientes":
                self.gestor_contexto_5410.cambiar_estado("pendientes_mostrados")
                return self._respuesta_contextual_5410(
                    "pendientes_mostrados",
                    formatear_pendientes_5413(natural.get("acciones") or []),
                    pendientes=natural.get("acciones") or [],
                )
            if tipo_natural in {"referencia_ordinal", "otra_accion"}:
                accion = natural.get("accion") or {}
                self.gestor_contexto_5410.cambiar_estado(
                    "esperando_confirmacion_especifica", accion_seleccionada=accion
                )
                return self._respuesta_contextual_5410(
                    "esperando_confirmacion_especifica",
                    mensaje_confirmacion_especifica_5412(accion),
                    accion=accion,
                    intencion="seleccion_flujo",
                )
            if tipo_natural == "referencia_ultima_accion":
                # "hazlo" confirma la última acción seleccionada.
                accion = natural.get("accion") or {}
                ...
                return self._respuesta_contextual_5410(
                    "accion_confirmada_modo_seguro",
                    formatear_resultado_seleccion(resultado, contexto.get("datos_flujo") or {}),
                    resultado=resultado,
                    intencion="seleccion_flujo",
                )

        interpretada = interpretar_confirmacion_contextual_5412(texto, contexto)
        if interpretada.get("gestionado"):
            tipo = interpretada.get("tipo")
            if tipo == "abrir_seleccion":
                acciones = acciones_seleccionables(contexto)
                self.gestor_contexto_5410.cambiar_estado("esperando_seleccion_accion")
                lineas = ["Selecciona qué acción quieres continuar:"] + [
                    f"{i}. {a.get('nombre')}" for i, a in enumerate(acciones, 1)
                ]
                return self._respuesta_contextual_5410(
                    "seleccion_acciones_pendiente", "\n".join(lineas), acciones=acciones
                )
            if tipo in {"seleccionar_accion", "seleccionar_y_confirmar"}:
                accion = interpretada.get("accion") or {}
                if tipo == "seleccionar_y_confirmar" or not bool(accion.get("modifica_datos")):
                    resultado = ejecutar_accion_seleccionada_modo_seguro(accion, contexto)
                    registro = registrar_confirmacion_accion_5412(contexto, accion, True)
                    self.gestor_contexto_5410.cambiar_estado(
                        "accion_confirmada_modo_seguro",
            ...
            if tipo == "confirmar_accion":
                accion = contexto.get("accion_seleccionada") or {}
                resultado = ejecutar_accion_seleccionada_modo_seguro(accion, contexto)
                registro = registrar_confirmacion_accion_5412(contexto, accion, True)
                self.gestor_contexto_5410.cambiar_estado(
                    "accion_confirmada_modo_seguro",
            ...
            if tipo in {"volver_seleccion", "rechazar_accion"}:
                self.gestor_contexto_5410.cambiar_estado(
                    "esperando_seleccion_accion", accion_seleccionada=None
                )
                return self._respuesta_contextual_5410(
                    "seleccion_acciones_pendiente", "He vuelto a la selección de acciones."
                )
            if tipo == "cancelar_flujo":
                anterior = self.gestor_contexto_5410.finalizar("cancelado_por_usuario")
                self.contexto_activo_549 = self.gestor_contexto_5410.contexto
                return self._respuesta_contextual_5410(
                    "flujo_cancelado", "Flujo cancelado. No se han modificado datos reales.", anterior=anterior
                )
            if tipo == "confirmar_flujo":
                return self._continuar_flujo_confirmado({"tipo": "confirmar_todo", "texto": texto})

        if self.gestor_contexto_5410.estado() == "esperando_seleccion_accion":
            seleccion = interpretar_seleccion(texto, acciones_seleccionables(contexto))
            if seleccion.get("tipo") == "accion":
                accion = seleccion.get("accion") or {}
                resultado = ejecutar_accion_seleccionada_modo_seguro(accion, contexto)
                self.gestor_contexto_5410.cambiar_estado(
                    "esperando_confirmacion_especifica" if accion.get("modifica_datos") else "accion_confirmada_modo_seguro",
                    accion_seleccionada=accion,
                )
                return self._respuesta_contextual_5410(
                    "esperando_confirmacion_especifica" if accion.get("modifica_datos") else "accion_confirmada_modo_seguro",
                    mensaje_confirmacion_especifica_5412(accion) if accion.get("modifica_datos") else formatear_resultado_seleccion(resultado, contexto.get("datos_flujo") or {}),
                    resultado=resultado,
                    intencion="seleccion_flujo",
                )
        return None
    ...
    def _hay_flujo_pendiente_confirmacion(self) -> bool:
        return bool(self._sincronizar_contexto_5410().get("flujo_pendiente_confirmacion"))
    ...
    def _procesar_evento_completo(self, datos_evento: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.gestor_contexto_5410.activar_flujo({
    ...
    def _continuar_flujo_confirmado(self, confirmacion: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.gestor_contexto_5410.cambiar_estado(
            "flujo_revisado_esperando_siguiente_accion",
            flujo_pendiente_confirmacion=False,
            ejecucion_confirmada=ejecucion,
        )
        self.contexto_activo_549 = self.gestor_contexto_5410.contexto
