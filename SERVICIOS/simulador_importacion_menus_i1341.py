...
class SimuladorImportacionMenusI1341:
    """I1.3.4.1 — genera un plan de importación sin escribir datos de negocio."""
    ...
    def simular(self, ruta_excel: str | Path, hojas: list[str] | None = None) -> dict[str, Any]:
        antes = self._huellas()
        pre = self.preimportador.preparar(ruta_excel, hojas=hojas)
        plan = self.construir_plan(pre)
...
def formatear_simulacion_i1341(resultado: dict[str, Any]) -> str:
    ...
    lineas = [
        ...
        f"Menús: {r.get('menus',0)} | Acciones: {r.get('acciones',0)} | Ejecutables: {r.get('ejecutables',0)} | Bloqueadas: {r.get('bloqueadas',0)}",
        f"Crear: {r.get('crear',0)} | Actualizar: {r.get('actualizar',0)} | Vincular: {r.get('vincular',0)} | Relaciones: {r.get('relaciones',0)} | Registrar: {r.get('registrar',0)}",
    ]
    ...
    lineas.extend([
        ...
        "I1.3.4.1 no dispone de confirmación ni botón de importación.",
        "El plan queda preparado para I1.3.4.2 — Transacción segura.",
    ])
    return "\n".join(lineas)
