...
def create_app(platform_api: HostAIPlatformAPI | None = None) -> FastAPI:
    ...
    @app.get("/api/v1/produccion/planes/{plan_id}/tareas/{tarea_id}/consumo-previsto")
    async def get_consumo_previsto(plan_id: str, tarea_id: str, request: Request) -> JSONResponse:
        return await _delegate(request)

    @app.post("/api/v1/produccion/planes/{plan_id}/tareas/{tarea_id}/confirmar")
    async def post_confirmar_produccion(plan_id: str, tarea_id: str, request: Request) -> JSONResponse:
        return await _delegate(request, body=await _json_body(request))
