...
def ejecutar_prueba():
    ...
    c = core.orquestador.resolver(SolicitudHostAI('comprobar_cierre_escandallos_inteligentes', {'escandallos': ESCANDALLOS, 'historico': HISTORICO, 'ventas': VENTAS})); assert c.ok, c.mensaje
    assert c.datos['version'] == '3.0.7.8'
    assert len(c.datos['modulos_validados']) >= 7
    assert c.datos['metricas']['total_escandallos'] == 2
    print('TEST OK - Host AI 3.0.7.8 Cierre Escandallos Inteligentes')
    print('Módulos validados:', len(c.datos['modulos_validados']))
    print('Estado:', c.datos['estado_general'])
