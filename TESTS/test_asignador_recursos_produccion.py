...
def ejecutar_prueba():
    ...
    r = core.orquestador.resolver(SolicitudHostAI('asignar_recursos_produccion', {'elaboraciones': ELABORACIONES, 'cocineros': 3, 'jornada_horas': 7.5}))
    assert r.ok, r.mensaje
    assert r.datos['version'] == '3.0.6.4'
    assert r.datos['total_asignaciones'] == 3
    assert r.datos['cocineros'] == 3
    print('TEST OK - Host AI 3.0.6.4 Asignador Inteligente de Recursos y Cocineros')
    print('Asignaciones:', r.datos['total_asignaciones'])
    print('Conflictos:', len(r.datos['conflictos']))
