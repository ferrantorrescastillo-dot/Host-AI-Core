...
def ejecutar_prueba():
    ...
    control = core.orquestador.resolver(SolicitudHostAI('controlar_ejecucion_produccion', {'planificacion': plan.datos, 'avance': [{'clave': plan.datos['bloques'][0]['clave'], 'avance_pct': 0, 'estado': 'bloqueada'}], 'minuto_actual': 180}))
    r = core.orquestador.resolver(SolicitudHostAI('replanificar_inteligente_produccion', {'control': control.datos, 'cocineros_disponibles': 3, 'incidencias': [{'descripcion':'Horno saturado','gravedad':'critico','impacto_min':45,'recurso':'horno'}]}))
    assert r.ok, r.mensaje
    assert r.datos['version'] == '3.0.6.6'
    assert r.datos['total_acciones'] >= 1
    assert len(r.datos['nuevo_orden']) >= 1
    print('TEST OK - Host AI 3.0.6.6 Replanificador Inteligente de Producción')
