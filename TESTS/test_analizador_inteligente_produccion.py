...
def ejecutar_prueba():
    ...
    r = core.orquestador.resolver(SolicitudHostAI('analizar_inteligente_produccion', {'elaboraciones': ELABORACIONES, 'jornada_horas':7.5, 'cocineros':3}))
    assert r.ok, r.mensaje
    assert r.datos['version'] == '3.0.6.1'
    assert r.datos['total_elaboraciones'] == 3
    assert r.datos['tiempo_activo_total_min'] >= 200
    assert 'horno' in r.datos['recursos_detectados']
    print('TEST OK - Host AI 3.0.6.1 Analizador Inteligente de Producción')
    print('Elaboraciones:', r.datos['total_elaboraciones'])
