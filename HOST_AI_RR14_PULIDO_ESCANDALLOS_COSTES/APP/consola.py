...
class AppConsolaHostAI:
    ...
    def __init__(self, core):
        ...
        self.ultima_necesidad_compra_id = None
        self.ultimo_pedido_compra_id = None
        self.ultimo_escandallo_id = None
    ...
    def _continuar_evento(self, origen):
        ...
        print("1. Añadir o gestionar otro pase/servicio")
    ...
    def _menu_compras(self):
        while True:
            pendientes = len(self.core.compras.listar_necesidades(solo_pendientes=True))
            abiertos = len([p for p in self.core.compras.listar_pedidos() if p.get("estado") not in {"recibido", "cancelado"}])
            print(f"\nGESTIÓN DE COMPRAS — {pendientes} necesidad(es) pendiente(s) | {abiertos} pedido(s) abierto(s)")
    ...
    def _recibir_pedido_compra(self):
        ...
        try:
            ...
            print(r.mensaje)
            if r.ok:
                print("Resumen de recepción:")
                for entrada in r.datos.get("entradas_stock", []):
                    nombre = entrada.get("nombre") or entrada.get("articulo") or "Artículo"
                    cantidad = entrada.get("cantidad", "-")
                    unidad = entrada.get("unidad", "")
                    print(f"- {nombre}: +{cantidad} {unidad} en {ubicacion}")
                print("Las necesidades vinculadas han quedado marcadas como compradas.")
        except Exception as exc:
    ...
    def _menu_costes(self):
        while True:
            print(f"\nCOSTES | Precios registrados: {self.core.costes_inteligente.listar_precios().get('total', 0)}")
